...
$passwordOfUbuntu = "Zwz@ms6165311"  # CHANGE THIS
...
if ($pscpPath) {
    # Using pscp - FIXED: Added $ before passwordOfUbuntu
    ...
    if ($scpPath) {
        ...
        if (Get-Module -ListAvailable -Name Posh-SSH) {
            ...
            try {
                Set-SCPItem -ComputerName "20.6.8.1" -Credential $credential -Path "$tarFilePath" -Destination "./" -AcceptKey -ErrorAction Stop -Verbose
...
"@
            ...
            if ($wslPath) {
                ...
                # FIXED: Proper quoting for password with spaces
...
if ($transferSuccess -or $LASTEXITCODE -eq 0) {
